...
def optimal_behavior_trajectories(t, y, params, seasons = False, taun_scared=1, taun_bold = 1, taup=1):
    ...
    cmax, mu0, mu1, eps, epsn, cp, phi0, phi1, cbar, lam = params.values()
    if seasons is True:
        Cdot = lam*(cbar+0.5*cbar*np.cos(t*np.pi/6) - C) - cmax*(N_scared*taun_scared*C/(taun_scared*C+cmax) + N_bold*taun_bold*C/(taun_bold*C+cmax)) #12t gives one whole year, 1t is one month
    else:
    ...
    N_scareddot = N_scared*(epsn*cmax*taun_scared*C/(taun_scared*C+cmax) - mu0*taun_scared - mu1 - cp*taup*taun_scared/(N_scared*taup*taun_scared+N_bold*taup*taun_bold + cp))

    print(epsn*cmax*taun_scared*C/(taun_scared*C+cmax) - mu0*taun_scared - mu1 - cp*taup*taun_scared/(N_scared*taup*taun_scared+N_bold*taup*taun_bold + cp))
    #Well, still weird. Maybe remove the option to eat the scared guys after all?? All very weird. #, removed them from the pool now. I guess the idea was they didn't get eaten at all.
    N_bolddot = N_bold*(epsn*cmax*taun_bold*C/(taun_bold*C+cmax) - cp*taup*taun_bold*N_bold/(N_scared*taup*taun_scared+N_bold*taup*taun_bold + cp) - mu0*taun_bold - mu1)
