...
#    print(taup(border_of_comp))
    print(max(taun_fitness_II(com_num, y, params)), C, N, P, params)
...
#    print(max_cands_two[np.argmax(taun_fitness_II(max_cands_two))], border_of_comp)
    ...
    return border_of_comp, max_cands_two[np.argmax(taun_fitness_II(max_cands_two, y, params))]
...
def optimal_behavior_trajectories(t, y, params, opt_prey = True, opt_pred=True, seasons = False, taun_scared=1, taun_bold = 1, taup=1):
    ...
    N_scared = y[1]
    N_bold = y[2]
    N_tot = y[1]+y[2]       #taun_scared*y[1]+taun_bold*y[2]
    rat_s = N_scared/N_tot  #taun_scared*N_scared/N_tot4
    ...
    N_scareddot = N_scared*(epsn*cmax*taun_scared*C/(taun_scared*C+cmax) - rat_s*cp*taup*taun_scared*N_scared/(rat_s*N_scared*taup*taun_scared + cp) - mu0*taun_scared - mu1) #Well, still weird. Maybe remove the option to eat the scared guys after all?? All very weird.
...
print(opt_taup_find(sol[:,-1], numbs[np.argmax(taun_fitness_II(numbs, sol[:,-1], params_ext))], params_ext))
