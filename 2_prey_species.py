...
def opt_taun_find(y, params):
    ...
    border_of_comp = com_num[np.where(max_num > 0)[0][0]] - 0.001
    max_cands_two = np.zeros(4)

    max_cands_two += border_of_comp*5
    linsp = np.linspace(border_of_comp*5 , 1, 100)
    ...
    if len(np.where(comparison_numbs > 0)[0]) is 0 or len(np.where(comparison_numbs < 0)[0]) is 0:
        max_cands_two[1] = linsp[np.argmax(taun_fitness_II(linsp))] #1
    ...
    print(max_cands_two[np.argmax(taun_fitness_II(max_cands_two))])

    return border_of_comp, max_cands_two[np.argmax(taun_fitness_II(max_cands_two))]


def optimal_behavior_trajectories(t, y, params, opt_prey = True, opt_pred=True, seasons = False, taun_scared=1, taun_bold = 1, taup=1):
    C = y[0]
    N_scared = y[1]
    N_bold = y[2]
    N_tot = y[1]+y[2]
    rat_s = N_scared/N_tot
    rat_b = N_bold/N_tot

    P = y[3]
    cmax, mu0, mu1, eps, epsn, cp, phi0, phi1, cbar, lam = params.values()
    if seasons is True:
        Cdot = lam*(cbar+0.5*cbar*np.cos(t*np.pi/180) - C) - cmax*(N_scared*taun_scared*C/(taun_scared*C+cmax) + N_bold*taun_bold*C/(taun_bold*C+cmax)) # t is one month
    else:
    ...
    N_scareddot = N_scared*(epsn*cmax*taun_scared*C/(taun_scared*C+cmax) - mu0*taun_scared - mu1 - rat_s*cp*taup*taun_scared*N_scared/(rat_s*N_scared*taup*taun_scared + cp))
    ...
    Pdot = P*(rat_b * cp*eps*taup*taun_bold*N_bold/(rat_b*N_bold*taup*taun_bold + cp) + rat_s*cp*eps*taup*taun_scared*N_scared/(rat_s*N_scared*taup*taun_scared + cp) - phi0*taup - phi1)
